...
export default function Usersidebar({users}) {
 ...
 const user=JSON.parse(localStorage.getItem("user"));
  ...
  const menuItems=[
      ...
      (<div className="rounded-full object-cover w-12 h-12"><img src={`http://localhost:5000${users.profilePicture}`} className="w-12 h-12 rounded-full object-cover"/></div>),label:user.isProfile?(<div className="text-white text-xl">{users.profile.fullName}</div>):"Profile",link:"/user/profileme"},
