...
const StepThree = ({nextStep,prevStep,formdata,setFormdata}) => {
    const {register,handleSubmit,formState:{errors}}=useForm({defaultValues:{
         title:formdata.steps[1].title,
        description:formdata.steps[1].description,
        checklist:formdata.steps[1].checklist,
        resources:formdata.steps[1].resources,
        mentortips:formdata.steps[1].mentortips,
    }});
    const onSubmit=(data)=>{
        
        const updateSteps=[...formdata.steps];
        updateSteps[1]={...updateSteps[1],...data};
